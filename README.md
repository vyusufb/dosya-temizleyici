...
## 🗑️ Dosyaları Kalıcı Olarak Yok Etme (Manuel İmha)

Ultra Cleaner V8, güvenlik protokolü gereği dosyaları **SİLMEZ**, sadece **HAPSEDER** (Karantinaya alır). Bu, yanlış silinen verilerin kurtarılabilmesi içindir.

Disk alanını tamamen geri kazanmak ve dosyaları kalıcı olarak yok etmek için aşağıdaki adımları uygulayın:

1.  Temizlik yaptığınız klasöre gidin.
2.  Gizli dosyaları görünür yapın:
    * **Windows:** *Görünüm > Göster > Gizli Öğeler* seçeneğini işaretleyin.
    * **Linux/Mac:** `ls -a` komutunu kullanın veya `Ctrl+H` kısayolunu deneyin.
3.  `.karantina_{TARIH}_{SAAT}` (Örn: `.karantina_20260218_123000`) isimli klasörü bulun.
4.  Bu klasörü seçin ve **Shift + Delete** tuşlarına basarak (veya `rm -rf` komutuyla) kalıcı olarak silin.

**⚠️ UYARI:** Karantina klasörü silindikten sonra veriler **kurtarılamaz**.
